...
export class FirmwareUi {
  ...
  perf(): { sceneUs: number; worstUs: number; nodes: number; clips: number; pixels: number; missed: number } | null {
    const x = this.x;
    if (!x || !this.running || typeof x.kui_perf !== 'function') return null;
    return { sceneUs: x.kui_perf(0), worstUs: x.kui_perf(1), nodes: x.kui_perf(2), clips: x.kui_perf(3), pixels: x.kui_perf(4), missed: x.kui_perf(5) };
  }
