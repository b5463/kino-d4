...
| `SET_LINK_BAUD` | `0x45` | → `{ "baud": 1500000, "cam"?: "cam1", "persist"?: true }` ← **inline** `{ "ok": true, "baud": 1500000, "cams": [{ "cam", "ok", "baud", "error"? }] }`. `cam` omitted means all four; `ok` is false unless every channel asked for switched. Supported rates are 921600, 1500000, 2000000, 3000000. Refused `BUSY` while a capture is running. Both ends treat a switch as provisional and fall back to 921600 if no frame decodes at the new rate within ~2.5 s, so a failed switch always recovers on its own. A rate that every channel took is stored in `body.linkBaud` and restored at boot: the nodes do not share the body's reset, so the body looks for each node at the stored rate first and then at the others, and brings it back to the stored one. `persist` defaults true; `false` changes the wire rate without changing what the body comes up at, which is what a bench walking the rates wants. Storing happens only when every channel switched and no `cam` was named. With nothing stored the body drives the link to 3000000. Timeout 30 s: the fallback has to be allowed to happen |
