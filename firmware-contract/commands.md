...
| `SET_LINK_BAUD` | `0x45` | → `{ "baud": 1500000, "cam"?: "cam1" }` ← **inline** `{ "ok": true, "baud": 1500000, "cams": [{ "cam", "ok", "baud", "error"? }] }`. `cam` omitted means all four; `ok` is false unless every channel asked for switched. Supported rates are 921600, 1500000, 2000000, 3000000. Refused `BUSY` while a capture is running. Both ends treat a switch as provisional and fall back to 921600 if no frame decodes at the new rate within ~2.5 s, so a failed switch always recovers on its own. Timeout 30 s: the fallback has to be allowed to happen |
