 ...
 * call returns 0 without waiting and the caller read that as "host gone" and
 * dropped the frame. The ring is 4096 bytes (usb_link_init), which is why
 ...
 *
 * There was a second, unbounded writer here - usb_link_write(), a portMAX_DELAY
 * loop for responses. It is gone: nothing on this board may wait forever on a
 * host that stopped reading, because whatever lock the caller holds waits with
 * it (kdp_server.c, s_tx_lock). Its "host gone" test was dead code as well,
 * since a portMAX_DELAY write cannot return zero - it just does not return.
 */
