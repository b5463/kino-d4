...
**0.4.52, the camera reports itself.** Heartbeat to the Roll server every 45 s while a roll is joined, and an immediate re-init on `409 UPLOAD_NOT_OPEN` instead of twelve doomed retries. Bench image `cd844ee9640e` flashed to the app slot; partition table and NVS untouched; booted as session boot-165. Built in all three configurations (bench, default, radio) with no warnings, which is the only check that covers `roll_http.c` and the radio half of `roll_api.c` - the host suites cannot compile them. Operator check pending, and it needs a reachable server: that heartbeats land every 45 s and one appears within a second or two of the queue emptying, that a swept upload recovers in one re-init rather than parking, and that the extra 1 KiB reply buffer in `roll_http_put_file` costs nothing measurable on the 8 KiB worker task at the first drain.

**0.4.54, a halted queue still reports.** The heartbeat carries `uploadPaused` off the queue's own `halted` flag, and the call moved above the upload worker's eligibility gate - on 0.4.52 a halted queue sent nothing, so the one fault that never self-clears was invisible to the host. Bench image `3814a2217cb1` flashed to the app slot; partition table and NVS untouched; booted as session boot-167. Bench, default and radio all build without warnings, which is the only check that covers `roll_api.c` and `upload_queue.c` - the host suites cannot compile them. Host suites green, roll-queue 1483 checks. Operator check pending, and `uploadPaused: true` needs a genuinely refused credential on a body with a working radio, which no board here has yet: what can be checked without one is that heartbeats still arrive every 45 s and that the queue's own screen and the dashboard agree.
