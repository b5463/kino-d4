...
| Durable upload queue | **BENCH DONE 2026-08-30** — a capture reaches a Roll on the LAN backend: document 201, thumbnail first, original byte-identical, one row; survives the API going away (retry landed 13.5 s after it returned) and a P4 reboot with the upload pending (5.9 s, on 0.4.4 — 0.4.3 waited on the previous boot's clock, fixed `ec22ea3`). 34 stale records on the bench card retired, none relabelled. Capture wins the card over an upload. **C6-only reset (0.4.5 `C6_RESET_BENCH`): the job survives, the P4 keeps running, but the SDIO link is never re-established - recovery needed a P4 reboot. ROLL-C test 3 FAIL; the supervisor owes a re-establishment path** |
