...
|---|---|---|
| Every §P4-header assignment drives its function (CAM1–4 UART, SYNC_TRIGGER GPIO32, FLASH_EN GPIO28, CAM_PWR_EN GPIO31) | continuity + scope on each pin under firmware control | `d4-v1.json` gpio (`PROVISIONAL` → `VERIFIED`), issue #2 |
| Per-camera power switching: one shared enable vs individual channel pins | drive CAM_PWR_EN, probe each AO4407 channel | `docs/HARDWARE.md` §Camera power switching |
| GPIO35 spare usable without boot-strap conflict | boot with pin pulled both ways | issue #2 |
