...
The released field body ([`hardware/cad/KINO_FIELD_BODY/`](../hardware/cad/KINO_FIELD_BODY/README.md), design 0.1.4, [`ECN-0004`](../hardware/changes/ECN-0004-field-body-slab-split-and-cover.md)) is a PETG slab, **131 × 90 × 65.5 mm**, 74.5 mm over its lens bar. Width is the P4 module plus two 6 mm walls; height is the module plus the shutter pod. It prints support-free as a two-part chassis cut at the component-bay floor (four M2.5 × 16 along the joint), a 9 mm face shell (glued, aligned through the four lens bores with the printed `KINO_FACE_ALIGN_TOOL`), a vertical sliding lens cover that drops below the lenses to shoot (with a keeper block glued into the track's entry gap as its stop), and a tool-less dovetail rear door. No acrylic. Six of the parts carry the design version and a build serial engraved 0.4 mm deep, the version read from `hardware/manifest.json` when the meshes are built.

**Knowing where the lens cover is.** One way now. The shell used to be drilled for a TO-92S Hall switch with an N52 disc in the cover — authoritative and free to read, but it had to be fitted before the face shell was glued on, and that joint is permanent, which made it the only irreversible step in the assembly. **That geometry is removed** (design 0.1.4, `ECN-0004`): no pocket, no lead channel, no plate hole, no magnet, and the front plate's opening inventory is back to the four lens bores. Firmware 0.4.53 carries the only remaining method: with the cover closed all four lenses face an opaque plate 0.2 mm away, so the P4 reads a mean luminance off the viewfinder's own decoded frames and decides from all four together. That method is `MEASURE_REQUIRED` — its two thresholds have to be read at the bench with the cover on and off, because the nodes run auto-exposure and a covered sensor reports amplified noise rather than zero — and it ships observing only until they are. It can put the camera to sleep; it cannot wake it, because `camIdleTimeoutS` cuts the camera rail and a node needs one to two seconds to boot its sensor. It also cannot distinguish a closed cover from a dark room. The camera row is four Ø7.1 bores at 22.00 mm pitch on Y = 43 in the front plate, measured off the released mesh.

The measured inputs the body is built on, with the confidence labels used on this page: camera module PCB 17.9 × 15.5 × 3.5 mm long axis vertical, XIAO + Sense stack 7.6 mm, camera ribbon about 7 mm with its FPC connector on the boards' **outer** face — the same face the module folds back onto, so the module lies flat on the stack and its lens looks along the boards' normal — 12.5 mm between the XIAO header rows, P4 brass standoffs 3.3 mm tall Ø3.5 on a **65 × 58 mm edge-to-edge** span, which is a **61.9 × 54.8 centre pitch** (the drawing's figure, confirmed by caliper). The 65.5 × 58.2 that stood here was that same edge span written into a centre-pitch field, and it moved every post 1.8 mm out until the printed posts splayed, lens barrel Ø7.0, light base plate 18 mm square on a 25 mm stud — all `MEASURED`. That pattern's **9.3 mm offset from module centre** is not: it is `PROVISIONAL`, and `coupons/KINO_P4_1TO1_HOLE_TEMPLATE.pdf` printed at 100 % is what checks it. Module outline 117.01 × 69.41 × 13.8 and active area 93.6 × 56.16 — `OFFICIAL_CAD`. The **assembled camera stack is 21 × 17.8 × 15 mm** — `OFFICIAL_SPEC`, Seeed's own product dimensions — and that 15.0 mm runs from the XIAO's back face to the **lens's front face**, fold and connector included. It is the figure the body's camera stations are built from: the board's stand-off is `15.0 − 7.6 of boards − 3.5 of pocket = 3.9 mm`, derived rather than chosen, because the ribbon leaves the camera nowhere else to be. A **chosen** 7.0 mm on the first printed wiggle rig left the lens about 2 mm short of its pocket. The lens cells are cut for a 78° diagonal field — a **design figure**; the OV3660 modules' actual field is `MEASURE_REQUIRED` (see the camera row above), and the released shells pass 81.9°, so any module up to about 81° diagonal is clear. Three inputs are still `PROVISIONAL` and named in the CAD README: the standoff pattern's 9.3 mm offset above, that the active area is centred on the PCB (the door window's 0.8 mm/side margin depends on it), and that field-of-view figure.
