...
| `recovery` phase swept through its consumers | 02 §6 | implemented now | `apps/studio/src/state/connectionStore.ts` (`canOpenDemo`) used at `Toolbar.tsx:110`; `apps/studio/src/state/benchResults.ts:146-155` invalidates bench results on `recovery` as well as `disconnected`/`error`. Tests: `specAudit.test.ts` → `02 §6 — recovery phase consumers` (3 cases) |
